...
<script>
	$(function(){
		$('#dg').datagrid({
		    height: '100%',
		    fit:true,
		    url: '<%=basePath %>framework/admin/getMyInfo.action',
		    method: 'POST',
		    striped: true,
		    nowrap: true,
		    pageSize: 10,
	...
	function saveBean(){
		...
		$.post("<%=basePath%>framework/admin/addMyInfo.action",{title:majorName,text:majorText},function(res,stutas){
			if(res==1){
	...
	function dele(){
		...
		var idstr = row[0].id;
		$.post("<%=basePath%>framework/admin/deletMyInfo.action",{idstr:idstr},function(res,stutas){
			if(res==1){
