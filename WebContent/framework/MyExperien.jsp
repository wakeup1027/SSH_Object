...
<script>
	$(function(){
		$('#dg').datagrid({
		    height: '100%',
		    fit:true,
		    url: '<%=basePath %>framework/admin/getExperien.action',
		    method: 'POST',
		    striped: true,
		    nowrap: true,
		    pageSize: 10,
	...
	function saveBean(){
		...
		$.post("<%=basePath%>framework/admin/addExperien.action",{title:majorName,text:majorText},function(res,stutas){
			if(res==1){
	...
	function dele(){
		...
		var idstr = row[0].id;
		$.post("<%=basePath%>framework/admin/deletExperien.action",{idstr:idstr},function(res,stutas){
			if(res==1){
...
<div id="dlg" class="easyui-dialog" style="width:450px; height:350px; padding: 10px 20px" closed="true" buttons="#dlg-buttons">
	<div style="line-height: 30px;  border-bottom: 1px solid #ccc;  margin-bottom: 15px;  font-size: 15px;">增加我的经历</div>
	<table>
